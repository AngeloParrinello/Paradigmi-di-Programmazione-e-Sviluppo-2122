...
object Student:
  ...
  private case class StudentImpl(name: String, year: Int) extends Student :
    ...
    // override def enrolling(course: Course*): Unit = for c <- course do coursesList = append(coursesList, Cons(c, Nil()))
    override def enrolling(course: Course*): Unit = course.foreach(course => coursesList = append(coursesList, Cons(course, Nil())))
