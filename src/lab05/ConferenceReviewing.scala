...
case class ConferenceReviewingImpl() extends ConferenceReviewing :
  ...
  override def orderedScores(article: Int)(question: Question): List[Int] = this.reviews.filter(_._1 == article)
                                                                                        .map(_._2(question))
                                                                                        .sorted

  override def averageFinalScore(article: Int): Double = this.reviews.filter(_._1 == article)
                                                                     .map(_._2(Question.FINAL))
                                                                     .sum
                                                                     .toDouble / this.reviews.count(_._1 == article)

  private def accepted(article: Int): Boolean = this.averageFinalScore(article) > 5.0 && this.reviews.filter(_._1 == article)
                                                                                                     .map(_._2(Question.RELEVANCE))
                                                                                                     .exists(_ >= 8)

  override def acceptedArticles: Set[Int] = this.reviews.map(_._1)
                                                        .distinct
  ...
  override def averageWeightedFinalScoreMap: Map[Int, Double] = this.reviews.map(_._1)
                                                                            .distinct
                                                                            .map(x => (x, this.averageWeightedFinalScore(x)))
                                                                            .toMap
